...
FEATURE_COLUMNS = [
    ...
    "jours_repos","diff_allocation","bonus_changement_ferrure",
]
...
def build_features(df: pd.DataFrame, jockey_encoder=None, discipline_encoder=None):
    df=df.copy()
    ...
    return df[FEATURE_COLUMNS].apply(pd.to_numeric,errors="coerce").fillna(0),jockey_encoder,discipline_encoder
